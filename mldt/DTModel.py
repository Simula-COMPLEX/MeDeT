...
class DeviceDataset(Dataset):
    ...
    def __init__(self, file_name=None, device_name=None, i_features=None):
        ...
        df.iloc[:, 0:i_features - 1].astype(float)
        x = df.iloc[:, 0:i_features - 1].values
        y = df.iloc[:, i_features - 1].values

        global feature_cols
        feature_cols = i_features - 1
...
def train_dtmodel(devices_names, devices_params, processed_datafiles, status_codes_map, ml_root_dir, ex_name, shots,
                  num_tasks, maml_lr, meta_lr, total_iterations, tps, adaption_steps):
    i_features = len(devices_params)

    # create initial tasksets for all files
    for datafile in processed_datafiles:
        create_data_task_sets(file_name=datafile, devices_names=devices_names, ways=1, shots=shots,
                              num_tasks=num_tasks,
    ...
    for datafile in processed_datafiles:
        ...
        taskset = create_data_task_sets(file_name=datafile, devices_names=devices_names, ways=ways, shots=shots,
                                        num_tasks=num_tasks,
                                        i_features=i_features)
        global feature_cols
        # create the model
        model = DTModel(infeatures=i_features - 1, outfeatures=o_features, dim=128)
...
# -----------------------------
def finetune_dtmodel(devices_names, devices_params, processed_datafiles, status_codes_map, ml_root_dir, ex_name, shots,
                     num_tasks, maml_lr, meta_lr, total_iterations, tps, adaption_steps):
    global feature_cols
    i_features = len(devices_params)
